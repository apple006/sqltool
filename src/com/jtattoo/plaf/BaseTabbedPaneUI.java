...
public class BaseTabbedPaneUI extends TabbedPaneUI implements SwingConstants {
	...
	protected Color selColors[];
	// protected boolean showClose;
	protected boolean isActive;
	protected boolean [] showClose = new boolean[0];
	protected Rectangle[] closeRects = new Rectangle[0];
	 ...
	 * extended BaseTabbedPaneUI to do their own layout, the UI uses the
	...
	private void uninstallTabContainer() {
		if (JTattooUtilities.getJavaVersion() >= 1.6) {
			...
			// Remove all the tabComponents, making sure not to notify the
			// tabbedpane.
	...
	protected void installKeyboardActions() {
		...
		if (scrollableTabLayoutEnabled()) {
			...
			tabScroller.popupMenuButton.setAction(am
					.get("scrollTabsPopupMenuAction"));
		}
	}
	...
	protected Color[] getTabColors(int tabIndex, boolean isSelected,
		...
		if ((tabIndex >= 0) && (tabIndex < tabPane.getTabCount())) {
			...
			if ((backColor instanceof UIResource)) {
				if (isSelected) {
					colorArr = AbstractLookAndFeel.getTheme()
							.getSelectedColors();
				...
				if (isSelected) {
					colorArr = ColorHelper.createColorArr(
							ColorHelper.brighter(backColor, 60), backColor, 20);
				} else if (isRollover && isEnabled) {
					colorArr = ColorHelper.createColorArr(
							ColorHelper.brighter(backColor, 80),
							ColorHelper.brighter(backColor, 20), 20);
				} else {
					colorArr = ColorHelper.createColorArr(
							ColorHelper.brighter(backColor, 40),
	...
	protected Color[] getContentBorderColors(int tabPlacement) {
		...
		selColors = AbstractLookAndFeel.getTheme().getSelectedColors();
	...
	protected Color getGapColor(int tabIndex) {
		if (isTabOpaque() || tabIndex == tabPane.getSelectedIndex()) {
			if ((tabIndex >= 0) && (tabIndex < tabCount)) {
				Color tabColors[] = getTabColors(tabIndex,
						tabIndex == tabPane.getSelectedIndex(), false);
	...
	public void paint(Graphics g, JComponent c) {
		...
		paintContentBorder(g, tabPlacement, selectedIndex, 0, 0, c.getWidth(),
				c.getHeight());

		if (!scrollableTabLayoutEnabled()) {
			paintTabArea(g, tabPlacement, selectedIndex);
		}
		int sepHeight = contentBorderInsets.bottom;
		...
		Color[] colors = ColorHelper.createColorArr(loColor, darkLoColor,
				sepHeight);
		if(tabPane.getTabCount()>0){
		Rectangle tabRect = rects[0];
		Rectangle tab1 = new Rectangle(tabRect.x, tabRect.height,
				tabPane.getWidth() - tabRect.x, tabPane.getHeight());
	...
	protected void paintTabArea(Graphics g, int tabPlacement, int selectedIndex) {
		...
		if (scrollableTabLayoutEnabled()
				&& tabScroller.scrollBackwardButton.isVisible()) {
			if ((tabPlacement == TOP) || (tabPlacement == BOTTOM)) {
				g.setClip(clipRect.x, clipRect.y, clipRect.width + 1,
						clipRect.height);
			} else {
...
//					tabPane.repaint();
		...
		if ((selectedIndex >= 0) && (selectedIndex < rects.length)
				&& getRunForTab(tc, selectedIndex) == 0) {
...
//				tabPane.repaint();
	...
	protected void paintTab(Graphics g, int tabPlacement, Rectangle[] rects,
		...
		try {
			...
			if (doPaintContent) {
				...
				paintText(g, tabPlacement, font, metrics, tabIndex, title,
						textRect, isSelected);
	...
	protected void paintFocusIndicator(Graphics g, int tabPlacement,
		...
		if (tabPane.isRequestFocusEnabled() && tabPane.hasFocus() && isSelected
				&& tabIndex >= 0 && textRect.width > 8) {
			g.setColor(AbstractLookAndFeel.getTheme().getFocusColor());
			BasicGraphicsUtils.drawDashedRect(g, textRect.x - 4,
					textRect.y + 1, textRect.width + 8, textRect.height);
		}
	}
	...
	protected void paintTabBorder(Graphics g, int tabPlacement, int tabIndex,
		...
		case BOTTOM:
			if (roundedTabs) {
				paintRoundedBottomTabBorder(tabIndex, g, x, y, x2, y2 - 1,
						isSelected);
			} else {
	...
	protected void paintRoundedTopTabBorder(int tabIndex, Graphics g, int x1,
			int y1, int x2, int y2, boolean isSelected) {
		Graphics2D g2D = (Graphics2D) g;
	...
	protected void paintTopTabBorder(int tabIndex, Graphics g, int x1, int y1,
			int x2, int y2, boolean isSelected) {
		int tc = tabPane.getTabCount();
		...
		if (leftToRight) {
			...
			if (!isSelected && (tabIndex == firstIndex)
	...
	protected void paintLeftTabBorder(int tabIndex, Graphics g, int x1, int y1,
			int x2, int y2, boolean isSelected) {
		Graphics2D g2D = (Graphics2D) g;
	...
	protected void paintRoundedBottomTabBorder(int tabIndex, Graphics g,
			int x1, int y1, int x2, int y2, boolean isSelected) {
		Graphics2D g2D = (Graphics2D) g;
		Object savedRederingHint = g2D
				.getRenderingHint(RenderingHints.KEY_ANTIALIASING);
		g2D.setRenderingHint(RenderingHints.KEY_ANTIALIASING,
	...
	protected void paintBottomTabBorder(int tabIndex, Graphics g, int x1,
			int y1, int x2, int y2, boolean isSelected) {
		int tc = tabPane.getTabCount();
		int currentRun = getRunForTab(tc, tabIndex);
		int lastIndex = lastTabInRun(tc, currentRun);
	...
	protected void paintRightTabBorder(int tabIndex, Graphics g, int x1,
			int y1, int x2, int y2, boolean isSelected) {
		Graphics2D g2D = (Graphics2D) g;
	...
	protected void paintTabBackground(Graphics g, int tabPlacement,
		...
		if (isTabOpaque() || isSelected) {
			...
			default:
				...
				if (isSelected) {
					Area clipArea = new Area(new RoundRectangle2D.Double(x, y,
							w, h + 4, d, d));
					Area rectArea = new Area(new Rectangle2D.Double(x, y, w,
							h + 4));
					clipArea.intersect(rectArea);
					clipArea.intersect(orgClipArea);
					g2D.setClip(clipArea);
					JTattooUtilities.fillHorGradient(g, colorArr, x, y, w,
							h + 4);
			...
			case BOTTOM:
				if (isSelected) {
					Area clipArea = new Area(new RoundRectangle2D.Double(x,
							y - 4, w, h + 4, d, d));
					Area rectArea = new Area(new Rectangle2D.Double(x, y - 2,
							w, h + 1));
					clipArea.intersect(rectArea);
					clipArea.intersect(orgClipArea);
					g2D.setClip(clipArea);
					JTattooUtilities.fillHorGradient(g, colorArr, x, y - 4, w,
							h + 4);
					g2D.setClip(savedClip);
				} else {
					Area clipArea = new Area(new RoundRectangle2D.Double(x,
			...
			case RIGHT:
				if (isSelected) {
					JTattooUtilities.fillHorGradient(g, colorArr, x - 2, y + 1,
							w + 2, h - 1);
				} else {
					JTattooUtilities.fillHorGradient(g, colorArr, x, y + 1,
							w + 1, h - 1);
				}
				break;
			}
	...
	protected void paintContentBorder(Graphics g, int tabPlacement,
		...
		int tabAreaHeight = calculateTabAreaHeight(tabPlacement, runCount,
				maxTabHeight);
		int tabAreaWidth = calculateTabAreaWidth(tabPlacement, runCount,
				maxTabWidth);
		...
		if (tabPane.isOpaque()) {
			...
			int wt = tabPlacement == TOP || tabPlacement == BOTTOM ? w
					: tabAreaWidth;
			int ht = tabPlacement == LEFT || tabPlacement == RIGHT ? h
					: tabAreaHeight;
		...
		if (isContentOpaque()) {
			...
			int wt = tabPlacement == LEFT || tabPlacement == RIGHT ? w
					- tabAreaWidth : w;
			int ht = tabPlacement == TOP || tabPlacement == BOTTOM ? h
					- tabAreaHeight : h;
		...
		if (hasInnerBorder()) {
			...
			if (sepHeight > 0) {
				...
				case TOP: {
					...
					int ys = y + tabAreaHeight - sepHeight;
					for (int i = 0; i < colors.length; i++) {
						g.setColor(colors[i]);
						 g.drawLine(x+1, ys + i, x + w-2, ys + i);///888
					}
					//
					//
					break;
		...
		// h);
		// paintContentBorderRightEdge(g, tabPlacement, selectedIndex, x, y, w,
		// h);
	}

	protected void paintScrollContentBorder(Graphics g, int tabPlacement,
			int selectedIndex, int x, int y, int w, int h) {
		Insets bi = new Insets(0, 0, 0, 0);
		if (tabPane.getBorder() != null) {
			// bi = tabPane.getBorder().getBorderInsets(tabPane);
		...
		if (tabPane.getTabPlacement() == TOP) {
			paintContentBorder(g, tabPane.getTabPlacement(),
					tabPane.getSelectedIndex(), x, y - bi.top, w, h);
		} else if (tabPane.getTabPlacement() == BOTTOM) {
			paintContentBorder(g, tabPane.getTabPlacement(),
					tabPane.getSelectedIndex(), x, y + bi.bottom, w, h);
		} else if (tabPane.getTabPlacement() == LEFT) {
			paintContentBorder(g, tabPane.getTabPlacement(),
					tabPane.getSelectedIndex(), x - bi.left, y, w, h);
		} else if (tabPane.getTabPlacement() == RIGHT) {
			paintContentBorder(g, tabPane.getTabPlacement(),
					tabPane.getSelectedIndex(), x + bi.right, y, w, h);
		}
	}
	...
	protected void assureRectsCreated(int tabCount) {
		...
		if (tabCount != rectArrayLen) {
			...
			boolean[] showColseArrary = new boolean[tabCount];
			
			System.arraycopy(showClose, 0, showColseArrary, 0,Math.min(showClose.length, tabCount));
			rects = tempRectArray;
			showClose = showColseArrary;
			for (int rectIndex = rectArrayLen; rectIndex < tabCount; rectIndex++) {
				rects[rectIndex] = new Rectangle();
				showClose[rectIndex] = false;
			}
		}

		int closerectArrayLen = closeRects.length;
		if (tabCount != closerectArrayLen) {
			Rectangle[] tempRectArray = new Rectangle[tabCount];
			System.arraycopy(closeRects, 0, tempRectArray, 0,
					Math.min(closerectArrayLen, tabCount));
			closeRects = tempRectArray;
			for (int rectIndex = closerectArrayLen; rectIndex < tabCount; rectIndex++) {
	...
	protected int calculateTabAreaHeight(int tabPlacement, int horizRunCount,
		...
		if (tabPlacement == JTabbedPane.TOP
			...
			return (horizRunCount > 0 ? horizRunCount
					* (maxTabHeight - overlay) + overlay + insets.top
					+ insets.bottom : 0);
		} else {
	...
	protected int calculateTabAreaWidth(int tabPlacement, int vertRunCount,
		...
		if (tabPlacement == JTabbedPane.LEFT
			...
			Insets insets = getTabAreaInsets(tabPlacement);
			int overlay = getTabRunOverlay(tabPlacement);
			return (vertRunCount > 0 ? vertRunCount * (maxTabWidth - overlay)
					+ overlay + insets.left + insets.right : 0);
		} else {
	...
	private static class ScrollTabsPopupMenuAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (src instanceof JTabbedPane) {
				...
				tabbedPane = (JTabbedPane) ((ScrollablePopupMenuTabButton) src)
						.getParent();
			} else {
				return; // shouldn't happen
			}
			...
			if (ui.scrollableTabLayoutEnabled()) {
				...
				for (int i = 0; i < tabbedPane.getTabCount(); i++) {
					...
					item.addActionListener(new ScrollTabsPopupMenuItemAction(
							tabbedPane, i));
				...
				int x = -popup.getWidth()
						+ ui.tabScroller.popupMenuButton.getWidth();
	...
	public class TabbedPaneLayout implements LayoutManager {
			...
			if (numChildren > 0) {
				...
				for (int i = 0; i < numChildren; i++) {
					...
					if (child == tabContainer) {
						...
						int tabContainerWidth = totalTabWidth == 0 ? cw
								: totalTabWidth;
						int tabContainerHeight = totalTabHeight == 0 ? ch
								: totalTabHeight;
			...
			if (JTattooUtilities.getJavaVersion() >= 1.6) {
				...
				for (int i = 0; i < tabPane.getTabCount(); i++) {
					...
					tabComponent
							.setBounds(
									x
											+ getTabLabelShiftX(tabPlacement,
													i, isSeleceted),
									y
											+ getTabLabelShiftY(tabPlacement,
													i, isSeleceted),
		...
		protected void normalizeTabRuns(int tabPlacement, int tabCount,
			...
			if (tabPlacement == TOP || tabPlacement == BOTTOM) {
				...
				// across the runs in order to make the runs look more
				// consistent.
				//
				...
				while (keepAdjusting) {
					...
					// Check if the run has enough extra space to fit the last
					// tab
			...
			if ((numChildren > 0) && (tc > 0)) {
				...
				for (int i = 0; i < numChildren; i++) {
					...
					if (child instanceof ScrollableTabViewport) {
						...
						case LEFT:
						case RIGHT:
							int totalTabHeight = rects[tc - 1].y
									+ rects[tc - 1].height;
						...
						case TOP:
						default:
							int totalTabWidth = rects[tc - 1].x
									+ rects[tc - 1].width;
							if (totalTabWidth > tw) {
								// Allow space for scrollbuttons
						...
						case RIGHT:
							...
							if (totalTabHeight > th) {
								...
								visible = true;
								bx = tabPlacement == LEFT ? tw - insets.left
										- tabAreaInsets.bottom - bsize.width
										: bounds.width - insets.left
												- bsize.width;
								by = dir == SOUTH ? bounds.height
										- insets.bottom - 2 * bsize.height - 2
										: bounds.height - insets.bottom - 3
												* bsize.height - 2;
							}
							break;
						...
						default:
							...
							if (totalTabWidth > tw) {
								...
								bx = dir == EAST ? bounds.width - insets.left
										- 2 * bsize.width - 2 : bounds.width
										- insets.left - 3 * bsize.width - 2;
								by = ty
										+ (th - bsize.height - tabAreaInsets.bottom)
										/ 2;
								if (tabPlacement == BOTTOM) {
						...
						case RIGHT:
							...
							if (totalTabHeight > th) {
								...
								bx = tabPlacement == LEFT ? tw - insets.left
										...
										: bounds.width - insets.left
												- bsize.width;
								by = bounds.height - insets.bottom
										- bsize.height;
							}
							break;
		...
		protected void calculateTabRects(int tabPlacement, int tabCount) {
			...
			for (int i = 0; i < tabCount; i++) {
				...
				if (!verticalTabRuns) {
					...
					rect.height = maxTabHeight/* - 2 */;
					closeRect.y = (rect.y + rect.height) / 2 - 7;
					closeRect.x = rect.x + rect.width - 20;
	...
	private class ScrollableTabSupport implements ChangeListener {
		...
		public void stateChanged(ChangeEvent e) {
			...
			case LEFT:
				...
				scrollForwardButton.setEnabled(leadingTabIndex < tc - 1
	...
	private class ScrollableTabPanel extends JPanel implements UIResource {
		...
		public void paintComponent(Graphics g) {
			...
			paintScrollContentBorder(g, tabPane.getTabPlacement(),
					tabPane.getSelectedIndex(), 0, 0, getWidth(), getHeight());
			paintTabArea(g, tabPane.getTabPlacement(),
	...
	public class ArrowButton extends JButton implements SwingConstants {
		...
		public ArrowButton(int direction) {
			...
			if (simpleButtonBorder) {
				...
				Color cHi = AbstractLookAndFeel.getTheme()
						.getControlHighlight();
		...
		public void paintTriangle(Graphics g, int x, int y, int size) {
			...
			Color enabledColor = AbstractLookAndFeel.getTheme()
					.getButtonForegroundColor();
			Color disabledColor = AbstractLookAndFeel.getTheme()
	...
	private class ScrollableTabButton extends ArrowButton implements
	...
	private class ScrollablePopupMenuTabButton extends ArrowButton implements
			UIResource, SwingConstants {
	...
	public class PropertyChangeHandler implements PropertyChangeListener {
		...
		public void propertyChange(PropertyChangeEvent e) {
			...
			if ("mnemonicAt".equals(name)) {
				...
				try {
					Component tabComponent = getTabComponentAt(((Integer) e
		...
		public void stateChanged(ChangeEvent e) {
			...
			if (JTattooUtilities.getJavaVersion() >= 1.4) {
				if (tabPane.getTabLayoutPolicy() == JTabbedPane.SCROLL_TAB_LAYOUT) {
					...
					if (index >= 0) {
						BaseTabbedPaneUI ui = (BaseTabbedPaneUI) tabPane
								.getUI();
						ui.tabScroller.scrollTabToVisible(
								tabPane.getTabPlacement(), index);
					}
				}
			}
			tabPane.revalidate();
	...
	public class MouseHandler extends MouseAdapter {
		...
		public void mousePressed(MouseEvent e) {
			...
			if (tabIndex >= 0 && closeRects[tabIndex].contains(e.getPoint())) {
				int closerectArrayLen = closeRects.length;
				if (tabCount != closerectArrayLen) {
					Rectangle[] tempRectArray = new Rectangle[tabCount];
					System.arraycopy(closeRects, 0, tempRectArray, 0,
				...
				isActive = false;
				showClose[tabIndex]=false;
				tabPane.remove(tabIndex);
//				showClose.remove(tabIndex);
			}

			// rolloverIndex = -1;
			// if (rolloverIndex != oldRolloverIndex) {
			// if ((oldRolloverIndex >= 0) && (oldRolloverIndex <
			// tabPane.getTabCount())) {
			// tabPane.repaint(getTabBounds(tabPane, oldRolloverIndex));
			//
			// }
			// if ((rolloverIndex >= 0) && (rolloverIndex <
			// tabPane.getTabCount())) {
...
//			isActive = false;
			...
			if (rolloverIndex != oldRolloverIndex) {
				if ((oldRolloverIndex >= 0)
						&& (oldRolloverIndex < tabPane.getTabCount())) {
//					tabPane.repaint(getTabBounds(tabPane, oldRolloverIndex));
					showClose[oldRolloverIndex]=false;
					tabPane.repaint();

				}
				if ((rolloverIndex >= 0)
						&& (rolloverIndex < tabPane.getTabCount())) {
//					tabPane.repaint(getTabBounds(tabPane, rolloverIndex));
					tabPane.repaint();
				}
				oldRolloverIndex = rolloverIndex;
	...
	public class MouseMotionHandler extends MouseMotionAdapter {
		...
		public void mouseMoved(MouseEvent e) {
			...
			isActive = false;
			for (int i = 0; i < showClose.length; i++) {
				showClose[i]=false;
			}
			rolloverIndex = getTabAtLocation(e.getX(), e.getY());
			if (rolloverIndex != -1
					&& closeRects[rolloverIndex].contains(e.getPoint())) {
//				isActive = true;
				showClose[rolloverIndex]=true;
//				tabPane.repaint(getTabBounds(tabPane, rolloverIndex));
				tabPane.repaint();
			} else {
				if (rolloverIndex != -1) {
					showClose[rolloverIndex]=false;
//					tabPane.repaint(getTabBounds(tabPane, rolloverIndex));
					tabPane.repaint();
				}
			}
...
//			tabPane.repaint();
	 ...
	 * htmlViews list, which is only allocated after the first time we run into
	 * an HTML tab. Note that this list is kept in step with the number of
	...
	private class TabContainer extends JPanel implements UIResource {
		...
		public void remove(Component comp) {
			...
			PropertyChangeListener[] listeners = comp
					.getPropertyChangeListeners();
	...
	private void paintCloseIcon(Graphics g, int tabIndex, int nowIndex) {
		...
		LazyImageIcon closeActive = new LazyImageIcon(
				"icons/notification-close-active.gif");
		rect.width = close.getIconWidth();
		rect.height = close.getIconHeight();
		if (tabPane.getSelectedIndex() == tabIndex) {

			if (showClose[tabIndex]) {
				closeActive.paintIcon(tabPane.getTabComponentAt(tabIndex), g,
						x, y);
			} else {
			...
			if (showClose[tabIndex]) {
				closeActive.paintIcon(tabPane.getTabComponentAt(tabIndex), g,
						x, y);
			} else if (rolloverIndex == tabIndex && (rolloverIndex >= 0)
					&& (rolloverIndex < tabPane.getTabCount())) {
				close.paintIcon(tabPane.getTabComponentAt(rolloverIndex), g, x,
						y);
			}
